...
//map the input bits to proximal dendrites
	...
	for(i=0;i<16;i++){
		for(j=0;j<16;j++){
			for(k=0;k<3;k++){
			//set the dendrite map values to those random values
				layers[i][j].inputmap[k][0]=(j+1)%16;
				layers[i][j].inputmap[k][1]=(i-2)%16;
			}
		}
	}
